  ...
  function backToMenu() {
    document.getElementById('session-footer-active').style.display = '';
    document.getElementById('session-footer-ended').style.display  = 'none';
    nav('home-screen');
  }
  ...
  async function saveTranscription() {
    ...
    const tutorName = TUTORS[activeTutor].name;
    const dateStr   = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
    const divider   = '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━';

    addLine('PARLA — Session Transcription', 16, true);
    y += 4;
    addLine(divider);
    addLine(`Date: ${dateStr}`);
    addLine(`Tutor: ${tutorName}`);
    addLine(divider);
    y += 4;
    ...
    y += 4;
    addLine(divider);
    addLine(`Generated by Parla v${APP_VERSION}`, 9);

    const fileName = `parla-session-${new Date().toISOString().slice(0, 10)}.pdf`;
    doc.save(fileName);
  }
